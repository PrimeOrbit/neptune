...
- **Azure AD Permissions**: Ensure you have the following **administrative
  permissions** in Azure AD to run this script:
...
- **Important**: Keep your **Tenant ID** and **Redirect URI** ready before
  running the script. You will be prompted to provide these details during the
  execution:

  - **Tenant ID**: A valid Azure AD Tenant ID.
  - **Redirect URI**: A valid HTTPS redirect URL for your application (e.g.,
    `https://yoursite.primeorbit.io/login`).

## How to Use the Script

1. **Download the Script**: Download the
   [`aad-app-registration.ps1`](https://prime-orbit-setup.s3.us-east-1.amazonaws.com/azure/aad-app/aad-app-registration.ps1)
...
4. **Provide Inputs**: During the script execution, you'll be prompted to
   ...
   - **Redirect URI**: A valid HTTPS redirect URL for the application (e.g.,
     `https://yoursite.primeorbit.io/login`).

5. **Wait for Completion**: The script will register the app, assign
   permissions, and output the details

**Note**: This script configures the Azure AD application to include and manage
the `email` optional claim in ID tokens.
[Reference doc](https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims?tabs=appui)
